...
public class Ex01_String {
	...
	*/
	...
	public static void equals() {
		...
		// equals 메소드
		// 비교하는 문자열(String)이 동일하면 true, 아니면 false 반환
		...
		// 대소문자는 무시함
		if(str1.equalsIgnoreCase(str2)) { // Case 대소문자를 말함 upper대문자 , lower소문자 Case
			System.out.println("Str1과 str2는 같은 문자열이다.");
		}else {
			System.out.println("Str1과 str2는 다른 문자열이다.");
		} 
	...
	public static void length() {
		...
		String str = "Hello\nWorld"; 
		int length = str.length();   // arr.lenth() : 필드호출 , str.lenth() : 메소드호출
	...
	public static void charAt() {
		...
		String name = "정병인";  // 캐릭터에서는 한 글자씩 밖에 못끊는다.
	...
	public static void indexOf() {
		...
		// indexOf 사용법
		// 1. indexOf(String str) : 인덱스 0부터 str을 검색  // 찾을 문자열
		// 2. indexOf(String str, int index) : 인덱스 index부터 str을 검색
		
		String slogan = "걱정한다고 걱정이 없어지면 걱정이 없겠네";
		
		int idx1 = slogan.indexOf("걱정");  // 인덱스 0
		int idx2 = slogan.indexOf("걱정", idx1 + 1); // 첫번째 걱정 다음 부터 찾아라 ; 0 .6
		int idx3 = slogan.indexOf("걱정", idx2 + 1);
		
		int idx4 = slogan.indexOf("장종찬");  // -1 정상
	...
	public static void lastIndexOf() {
		...
		// 나머지 특성은 indexOf와 동일 //없으면 -1 부터 반환//
		
		String slogan = "걱정한다고 걱정이 없어지면 걱정이 없겠네";
		
		int idx1 = slogan.lastIndexOf("걱정");
		
		int idx2 = slogan.lastIndexOf("장종찬");
	...
	public static void contains() {
		...
		// contains
		// 문자열이 지정된 CharSequence(String, char[] 등)를 포함하면 true 반환  CharSequence = String으로 생각해도 문제없음
	...
	public static void isEmpty() {
		...
		String str = "";  // " "; 빈 문자열이 아니다. 여기서 if(str.trim().isEmpty())를 하면 빈 문자열이된다.
		if(str.isEmpty()) {
	...
	public static void format() {
		...
		// 문자 형식 지정하기
		String str = "hi";
		System.out.println(String.format("%s", str));  // %s : 문자로 표시하시오.
	...
	public static void ex01() {
		...
		String ur1 = "https://comic.naver.com/webtoon/detail?titleId=758037&no=112&weekday=mon";
				
		String requestURI = ur1.substring(0, ur1.indexOf("?"));  //  https://comic.naver.com/webtoon/detail
		System.out.println(requestURI);
		
		String params = ur1.substring(ur1.indexOf("?") + 1);  // titleId=758037&no=112&weekday=mon"
	...
	public static void ex02() {
		...
		if(fullName.endsWith(".tar.gz")) {
			...
			extName = fullName.substring(fullName.lastIndexOf(".") + 1); // .jpg
		}
