...
public class MainClass {
		...
		if(dir.exists()) {
			...
			for(File file : list) {
				...
				if(file.isFile()) {
					...
					size =  new DecimalFormat("#,##0").format(file.length());					// #,##0 천단위 구분기호4
					fileCount++;
	...
	// 문제4. 사용자로부터 입력 받은 문자열을 C:\storage\diatry.txt 파일로 보내시오.
	...
	public static void ex04() {
		...
		File file = new File(dir, "diatry.txt");
		
		try (PrintWriter out = new PrintWriter(file)) { 
			
			for(int i = 0; i < sentences.length; i++) {
				out.println(sentences[i]);					// 줄 바꿈을 할 수있게하는 Writer
			} 
	...
	public static void ex05() {
		...
		sc.close();
			...
			// 생성 모드(언제나 새로만든다.)	new FileWriter(file)
